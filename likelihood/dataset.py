...
def collate_me_fusion(batch, plane_names=("u", "v", "w")):
    ...
    ys = []
    available = torch.zeros((len(batch), len(plane_names)), dtype=torch.float32)
    ...
    for bi, (c, f, y) in enumerate(batch):
        ...
        for pi, name in enumerate(plane_names):
            ...
            if mask.any():
                available[bi, pi] = 1.0
                coords_plane = c[mask][:, 1:]  # (N,2): (y,x)
                feats_plane = f[mask][:, :2]   # (N,2): (occ, logq)
            else:
                coords_plane = torch.zeros((1, 2), dtype=torch.int32)
                feats_plane = torch.zeros((1, 2), dtype=torch.float32)

            bcol = torch.full((coords_plane.shape[0], 1), bi, dtype=torch.int32)
            coords_by_plane[name].append(torch.cat([bcol, coords_plane], dim=1))
            feats_by_plane[name].append(feats_plane)

    inputs = {}
    for name in plane_names:
        coords = torch.cat(coords_by_plane[name], dim=0).contiguous()
        feats = torch.cat(feats_by_plane[name], dim=0).contiguous()
        inputs[name] = {"coords": coords, "feats": feats}

    y = torch.tensor(ys, dtype=torch.float32)
    return inputs, available, y
