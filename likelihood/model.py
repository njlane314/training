...
class ResBlock(nn.Module):
    def __init__(self, cin, cout, ks=(3, 3)):
        super().__init__()
        self.conv1 = ME.MinkowskiSubmanifoldConvolution(
            cin, cout, kernel_size=ks, dimension=2, bias=False
        )
        self.bn1 = ME.MinkowskiBatchNorm(cout)
        self.conv2 = ME.MinkowskiSubmanifoldConvolution(
            cout, cout, kernel_size=ks, dimension=2, bias=False
        )
...
class SparseUResNetEncoderClassifier(nn.Module):
    """
    2D residual encoder + global pooling head for event-level LLR.
    Input coords: (batch, y, x) with D=2 spatial dims (y, x).
    """
